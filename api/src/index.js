...
async function handleBootstrap(request, env) {
  ...
  // Only allow bootstrap if no users exist OR if this email doesn't exist
  const existingCount = await env.DB.prepare("SELECT COUNT(*) as c FROM users").first();
  if (existingCount?.c > 0) {
    // If users already exist, block bootstrap to reduce abuse.
...
async function getUserFromCookie(request, env) {
  ...
  // basic expiration check (jwt already includes exp)
...
function buildSessionCookie(env, value, maxAgeSeconds = 60 * 60 * 24 * 7) {
  // Secure requires HTTPS. Cloudflare gives you that.
  // SameSite=Lax is fine for typical same-site usage; if your API and Pages are different subdomains,
  // still OK for fetch with credentials, but you must use credentials:'include' in the browser.
  const parts = [
...
async function hashPasswordPBKDF2(password) {
  const iterations = 210000; // decent cost without killing a Worker
